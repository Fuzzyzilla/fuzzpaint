...
//! Always accessed via reference borrowed from a document queue's [state reader](crate::commands::queue::state).
use crate::FuzzID;
...
pub type DocumentID = FuzzID<DocumentNamespaceTy>;
pub enum StrokeLayerNamespaceTy {}
pub type StrokeLayerID = FuzzID<StrokeLayerNamespaceTy>;

pub struct BorrowedBlendGraph<'s> {
    // Todoooo
    _p: std::marker::PhantomData<&'s ()>,
}
pub struct BorrowedStrokeLayer<'s> {
    pub id: StrokeLayerID,
    pub strokes: &'s [ImmutableStroke],
}
pub struct BorrowedDocument<'s> {
