...
mod stroke_renderer {
    ...
    impl StrokeLayerRenderer {
        ...
        pub fn draw(&self, strokes: &[super::ImmutableStroke], renderbuf: &RenderData, clear: bool) -> AnyResult<()> {
            let (future, vertices, indirects) = self.gpu_tess.tess(strokes)?;
            let mut command_buffer = vk::AutoCommandBufferBuilder::primary(self.context.allocators().command_buffer(), self.context.queues().graphics().idx(), vulkano::command_buffer::CommandBufferUsage::OneTimeSubmit)?;

            command_buffer.begin_rendering(vulkano::command_buffer::RenderingInfo {
            ...
            .bind_descriptor_sets(vulkano::pipeline::PipelineBindPoint::Graphics, self.pipeline.layout().clone(), 0, self.texture_descriptor.clone())
